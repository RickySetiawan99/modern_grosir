...
class CatalogController extends Controller
{
    ...
    public function products(Request $request)
    {
        ...
        // Transform products to include tier pricing
        $products->getCollection()->transform(function($product) use ($tier) {
            $basePrice = $product->retail_price;
            
            // Check for product-specific tier price override
            ...
            $product->formatted_base_price = number_format($basePrice, 0, ',', '.');
            $product->formatted_your_price = number_format($discountedPrice, 0, ',', '.');
